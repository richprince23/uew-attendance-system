...
class StudentController extends Controller
{
    ...
    public function getEncodings(Request $request)
    {
        try {
            ...
            // Check if the encoding already exists for the given student ID
            $existingRecognition = Recognitions::where('student_id', $sid)->first();

            if ($existingRecognition) {
                return response()->json([
                    'success' => false,
                    'message' => 'Facial recognition data already exists for this student.',
